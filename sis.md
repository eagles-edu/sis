...
- Student vocabulary validation now uses local CMUdict for pronunciation/primary stress and Merriam-Webster Collegiate-then-Learner's for authoritative written division. The protected Library pages are `/student/library.html` (student chat enabled) and `/admin/library` (admin chat disabled), with `/admin/library/manage` and `/admin/library/engagement` child routes.
