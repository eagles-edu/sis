...
test("news review modal supports student-scoped navigation and modal review actions", async () => {
  ...
  await waitFor(() => {
    ...
    assert.equal(document.querySelectorAll(".news-review-vocabulary-row span").length, 0)
    assert.equal(document.querySelector(".news-review-vocabulary-definition"), null)
    assert.equal(document.querySelectorAll(".news-vocabulary-lookup").length, 0)
...
test("overview anonymous exercise submissions panel renders and supports show-all toggle", async () => {
  ...
  const dom = await createAdminUiDom(async (resource, init = {}) => {
    ...
    if (pathname === "/api/admin/runtime/health") {
      return jsonResponse(200, {
        status: "ok",
        lastVerifyOk: true,
        lastStoreOk: true,
...
test("table sort controls and column-click headers reorder grade/performance data", async () => {
  ...
  const attendanceSortReference = new Date()
  const attendanceSortDaysSinceSaturday = (attendanceSortReference.getDay() + 1) % 7
  const attendanceSortSaturday = isoDateOffset(
    localIsoDate(),
    -attendanceSortDaysSinceSaturday,
  )
  const attendanceSortDateA = attendanceSortSaturday
  const attendanceSortDateB = isoDateOffset(attendanceSortSaturday, 1)
    ...
    if (url.includes("/api/admin/students/stu-01")) {
      return jsonResponse(200, {
        ...
        attendanceRecords: [
          {
            id: "att-01",
            attendanceDate: attendanceSortDateA,
            className: "Class A",
            quarter: "q1",
            status: "present",
          ...
          {
            id: "att-02",
            attendanceDate: attendanceSortDateB,
            className: "Class B",
            quarter: "q1",
            status: "absent",
