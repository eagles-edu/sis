  ...
  async () => {
    ...
    try {
      ...
      const layout = await page.evaluate(() => {
        ...
        const footerNodes = Array.from(globalThis.document.querySelectorAll(".hub-footer:not(#authFooter) > p")).map((node) => {
          if (!(node instanceof globalThis.HTMLElement)) return null
          const box = node.getBoundingClientRect()
          return { x: Math.round(box.x), y: Math.round(box.y), w: Math.round(box.width) }
        }).filter(Boolean)
      ...
      assert.ok(layout.headerControls.length >= 2, "app header should still expose its controls")
