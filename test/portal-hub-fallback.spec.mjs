...
test("portal hub panels stay on the shared portal surface tokens", () => {
  ...
  assert.match(portalThemeCss, /body\.portal-hub-page \.hub-prefooter \{\s*align-items: center;\s*background: var\(--hub-surface-support\);\s*border-color: var\(--hub-surface-border\);\s*box-shadow: var\(--hub-surface-shadow\);\s*display: flex;\s*justify-content: space-between;/s)
