...
test("sync-and-restart-test-runtime skips Prisma refresh in public mode only", () => {
  ...
  assert.match(script, /if \[\[ "\$status" == "3" \]\]; then\s+log "\$\{label\} reported no changes; continuing"/)
  assert.match(script, /if \[\[ "\$status" == "1" \]\]; then\s+log "\$\{label\} reported warning-only changes; continuing"/)
  ...
  assert.match(script, /log "syncing test sis root into \$\{TEST_ROOT\}"/)
