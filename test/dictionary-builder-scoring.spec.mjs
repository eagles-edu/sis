...
test("Builder renders every datum with a manual path and POS dropdowns", () => {
  ...
  assert.doesNotMatch(workbench, /Complete the robot verification for \$\{unresolvedRobotDatum\[1\]\.provider\}[\s\S]*before applying/, "Robot-blocked candidates do not block Apply")
  assert.doesNotMatch(workbench, /Waiting for confirmed \$\{unresolvedManualDatum\[0\]\} data from its preferred source\(s\); use Retry preferred sources before applying a manual value\./, "Manual candidates do not block Apply")
  ...
  assert.match(workbench, /const hasAvailableRequiredPreferredDatum = \(currentSnapshot, datum\)/, "ET and TH retain their required preferred-datum retry path")
  assert.match(workbench, /selection\.value \?\? ""/, "Apply preserves numeric and structured selected datum values")
  assert.match(workbench, /if \(!appliedFields\.length\) return/, "A successful zero-change Apply remains visible instead of closing silently")
