...
test("student Check feedback follows the codified field-order directive", () => {
  ...
  assert.doesNotMatch(studentHtml, /Improve for Extra Points/)
  assert.doesNotMatch(studentHtml, /newsComplianceWarningBody/)
  assert.match(studentJs, /state\.newsWarningFields = \{\}/)
  assert.match(studentJs, /state\.newsWarningTasks = \[\]/)
  assert.match(studentJs, /setNewsComplianceModalCtaVisible\(hasFailed\)/)
  assert.match(submissions, /warningFields: \{\}/)
...
test("student Check summary uses singular grammar for one required fix", () => {
  assert.match(submissions, /const requiredFixCount = Object\.keys\(minimumRequirements\.failedFields \|\| \{\}\)\.length/)
  assert.match(submissions, /requiredFixCount === 1 \? "" : "es"/)
  assert.match(submissions, /requiredFixCount === 1 \? "s" : ""/)
})
...
test("student report persistence keeps report identity and exact syllabication", () => {
  ...
  assert.match(studentJs, /if \(options\?\.preserveForm !== true && !preserveDirtyForm\)/)
