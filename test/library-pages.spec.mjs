...
test("admin Library is a protected physical page under Administration without chat", () => {
  ...
  assert.match(sharedPortalTheme, /library-admin-entry summary > \.library-entry-word[\s\S]*?color:\s*#F4C45F[\s\S]*?font-weight:\s*800/)
  assert.match(sharedPortalTheme, /library-entry-accordion-summary > \.library-entry-word[\s\S]*?color:\s*#F4C45F[\s\S]*?font-weight:\s*800/)
  ...
  assert.match(sharedPortalTheme, /html\[data-theme="dark"\] body\.admin-portal-page\.library-admin-page :is\([\s\S]*?\.section-head > h1[\s\S]*?\.library-review-sidebar-heading > h3[\s\S]*?color:\s*#F4C45F !important[\s\S]*?font-weight:\s*800/)
  ...
  assert.match(sharedPortalTheme, /html\[data-theme="dark"\] body:is\(\.student-portal-page, \.admin-portal-page\) \.new-word-entry-definition > section > strong[\s\S]*?color:\s*#F4C45F !important/)
...
test("admin Library editor uses the shared New Words row renderer", () => {
  ...
  assert.match(sharedPortalTheme, /\.dictionary-builder-source-leds\s*\{[\s\S]*?border:\s*1px solid var\(--portal-border\)[\s\S]*?min-height:\s*52px/)
  assert.match(sharedPortalTheme, /\.dictionary-builder-source-leds\s*\{[^}]*flex-wrap:\s*wrap[^}]*min-height:\s*52px[^}]*overflow:\s*hidden/)
  assert.doesNotMatch(sharedPortalTheme, /\.dictionary-builder-source-leds\s*\{[^}]*overflow-x:\s*auto/)
  assert.match(sharedPortalTheme, /\.dictionary-builder-source-led\s*\{[\s\S]*?flex:\s*0 0 48px[\s\S]*?height:\s*48px[\s\S]*?width:\s*48px/)
