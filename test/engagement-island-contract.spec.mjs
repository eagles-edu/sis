...
test("engagement pages remain wired to the shared matrix and their own data endpoints", () => {
  ...
  assert.match(profile, /emailQueued: yes\(emailEvent\("queuedAt"\)\), emailQueuedAt: emailEvent\("queuedAt"\)/u)
  assert.match(profile, /emailClicked: yes\(brevoEvent\("clickedAt"\)\)/u)
  assert.match(profile, /linkClicked: yes\(invitationEvent\("clickedAt"\)\)/u)
  assert.match(profile, /actionCompleted: yes\(invitationEvent\("completedAt"\)\)/u)
  ...
  assert.match(read("server/student-admin-routes.mjs"), /profile-engagement[\s\S]*?where: \{ parentProfileInvitations: \{ some: \{\} \} \}/u)
  ...
  assert.doesNotMatch(libraryHtml, /<th>First open<\/th>|<th>Unique open<\/th>/u)
...
test("all engagement matrices keep their ID column visible while horizontally scrolled", () => {
  ...
  assert.match(matrix, /eventColumn\("Profile \(Submitted\)", "actionCompleted", "interaction"/u)
  assert.match(matrix, /eventColumn\("SIS-RX-CLICK", "linkClicked", "interaction"/u)
  assert.match(matrix, /title: "SIS profile", columns: interactionColumns/u)
  assert.match(matrix, /title: "Email delivery", columns: positiveColumns/u)
