...
test("dictionary scraper integrations preserve protected media and audit contracts", () => {
  ...
  for (const token of ["libraryLdocePreviewDialog", "libraryOxfordPreviewDialog", "libraryBritannicaPreviewDialog", "libraryMerriamWebsterPreviewDialog", "data-vocabulary-ldoce-preview", "data-vocabulary-oxford-preview", "data-vocabulary-britannica-preview", "data-vocabulary-merriam-webster-preview", "ldoceFormattedDefinition", "showModal", "Destructive Replace Selected", "Destructive Replace All", "Apply LDOCE", "Apply Oxford", "preload = \"none\"", "localLibraryMedia", "protected local Library media", "ldoce-apply", "oxford-apply", "britannica-apply", "merriam-webster-apply", "dictionaryBuilderSourceMatrix", "library-dictionary-builder-header", "Dictionary Builder", "data-dictionary-builder-section-summary", "data-dictionary-builder-source-matrix-slot", "dictionary-builder-tab-step", "dictionary-builder-candidate-list", "is-complete", "role=\"tablist\"", "dictionary-builder-audio-file-status", "Live file:", "No live file"]) assert.match(workbench, new RegExp(token.replace(/[.*+?^${}()|[\]\\]/gu, "\\$&")))
  ...
  assert.match(workbench, /const initialSourceEntries = \[\.\.\.sourceEntries\]\.sort\([\s\S]*?\)\.slice\(0, INITIAL_SOURCE_GROUP_SIZE\)/)
  assert.match(workbench, /INITIAL_SOURCE_PROVIDER_ORDER = \["britannica", "oxford_ame", "ldoce", "wordhelp", "merriam_webster_thesaurus"\]/)
  assert.match(workbench, /const sourceEntries = \(snapshot\.sources \|\| \[\]\)\.filter\(\(source\) => source\.sourceUrl\)/)
