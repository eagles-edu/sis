              ...
              currentHomework: [
                {
                  ...
                  assignmentBundleJson: {
                    items: [{ title: "Essay Draft", url: "https://example.com/essay-draft" }],
                  },
                },
              ],
              overdueHomework: [
                {
                  ...
                  status: "overdue",
                  assignmentBundleJson: {
                    items: [{ title: "Missed Exercise", url: "https://example.com/missed-exercise" }],
                  },
                },
              ],
              assignmentHistory: [
...
test(
  ...
  async () => {
    ...
    try {
      ...
      const currentEventState = await page.evaluate(() => {
        const eventTitle = globalThis.document.querySelector(".fc-event-homework-track:not(.fc-event-homework-overdue) .fc-event-title");
        const eventLink = eventTitle?.closest("a");
        const dayCell = eventTitle?.closest("[data-date]");
        return {
      ...
      assert.equal(currentEventState.title, "Essay Draft");
      assert.equal(currentEventState.href, "https://example.com/essay-draft");
      assert.equal(currentEventState.date, "2025-09-15");
      ...
      const overdueEventState = await page.evaluate(() => {
        ...
        const primaryLink = globalThis.document.querySelector("#studentDetailPrimaryList .detail-item-title a");
        return {
          text: title?.textContent || "",
          color: style?.color || "",
          eventHref: eventLink?.href || "",
          primaryHref: primaryLink?.href || "",
        };
      });
      assert.ok(overdueEventState.text);
      assert.match(overdueEventState.color, /rgb\(125,\s*0,\s*16\)/i);
      assert.equal(overdueEventState.eventHref, "https://example.com/missed-exercise");
      assert.equal(overdueEventState.primaryHref, "https://example.com/missed-exercise");

      await page.evaluate(() => {
        globalThis.document.querySelector('a[data-page-target="performance-reports"]')?.click();
      });
      await page.waitForFunction(() => /Performance Reports SYTD Archive/i.test(globalThis.document.getElementById("studentDetailTitle")?.textContent || ""));
      const reportEventState = await page.evaluate(() => {
        const eventTitle = globalThis.document.querySelector(".fc-event-report .fc-event-title");
        const eventLink = eventTitle?.closest("a");
        const dayCell = eventTitle?.closest("[data-date]");
        return {
          title: eventTitle?.textContent || "",
      ...
      assert.match(reportEventState.title, /Report\s*\|\s*A2 Flyers/i);
      assert.equal(reportEventState.href, "/student/reports/a2-flyers-q1-report-1");
      assert.equal(reportEventState.date, "2026-03-10");
      ...
      assert.ok(calendarState.width > 0);
      assert.ok(calendarState.height > 0);
...
test(
  ...
  async () => {
    ...
    try {
      ...
      assert.match(complianceState.requiredBodyText, /Source link/i);
      assert.match(complianceState.requiredBodyText, /Author \/ byline/i);
      assert.doesNotMatch(complianceState.requiredBodyText, /__compliance/i);
    } finally {
      if (page) {
        await page.close().catch(() => {});
      }
...
test(
  ...
  async () => {
    ...
    try {
      ...
      await page.waitForFunction(() => (
        globalThis.document.getElementById("sourceLink")?.value === "https://www.bbc.com/news/articles/cy91vrzxn34o" &&
      ...
      assert.match(complianceState.summaryText, /revision/i);
      assert.match(complianceState.requiredBodyText, /Source link/i);
      assert.match(complianceState.requiredBodyText, /Title/i);
      assert.doesNotMatch(complianceState.requiredBodyText, /__compliance/i);
    } finally {
      if (page) {
        await page.close().catch(() => {});
