...
test("queue hub source contract includes student-week news-set panel", () => {
  ...
  assert.match(routes, /reviewCheckStatus/)
  assert.match(routes, /reviewCheckColor/)
  assert.match(routes, /"incomplete"/)
  assert.match(routes, /"waiting-revise"/)
})

test("news review check-state and revise chip label keep locked admin ui rules", () => {
  const html = fs.readFileSync(new URL("../web-asset/admin/student-admin.html", import.meta.url), "utf8")
  const checkStatusStart = html.indexOf("function newsReviewCheckStatusToken(")
  assert.ok(checkStatusStart >= 0, "newsReviewCheckStatusToken is present")
  const checkStatusChunk = html.slice(checkStatusStart, checkStatusStart + 1000)
  assert.match(checkStatusChunk, /if \(submitted > 0\) return "unchecked";/)
  assert.match(checkStatusChunk, /if \(revisionRequested > 0\) return "waiting-revise";/)
  assert.match(checkStatusChunk, /return "checked";/)
  assert.match(html, /if \(normalized === "revision-requested"\) return "Revise";/)
})
