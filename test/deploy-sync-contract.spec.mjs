...
const testMirrorNginxConfigPath = path.resolve(process.cwd(), "deploy/nginx/test.eagles.edu.vn.conf")
...
const testMirrorNginxConfig = fs.readFileSync(testMirrorNginxConfigPath, "utf8")
...
test("sync-and-restart-test-runtime pins the test env contract and mirrors the portal hub safely", () => {
  ...
  assert.match(testRuntimeSyncScript, /TEST_LOCAL_UI_RUNTIME_PARITY_MAP=\(/)
  assert.match(testRuntimeSyncScript, /verify_local_ui_runtime_parity\(\)/)
  assert.match(testRuntimeSyncScript, /local UI parity mismatch/)
})
...
test("sync-and-restart-test-runtime copies persisted admin UI settings into the test runtime", () => {
  ...
  assert.match(testRuntimeSyncScript, /web-asset\/images\/new-words\.png\|web-asset\/images\/new-words\.png/)
  ...
  assert.match(testRuntimeSyncScript, /\$\{target_public_root\}\/web-asset\/images\/new-words\.png/)
...
test("test-mirror vhost enables fast gzip compression for HTML and static assets", () => {
  assert.match(testMirrorNginxConfig, /gzip on;/)
  assert.match(testMirrorNginxConfig, /gzip_static on;/)
  assert.match(testMirrorNginxConfig, /gzip_min_length 128;/)
  assert.match(testMirrorNginxConfig, /gzip_comp_level 4;/)
  assert.match(testMirrorNginxConfig, /gzip_proxied any;/)
  assert.match(
    testMirrorNginxConfig,
    /gzip_types[\s\S]*text\/javascript[\s\S]*application\/xml\+rss[\s\S]*application\/manifest\+json/,
  )
  assert.doesNotMatch(testMirrorNginxConfig, /gzip_types[\s\S]*text\/html/)
  assert.doesNotMatch(testMirrorNginxConfig, /gzip_comp_level 9;/)
})

test("test-mirror vhost enables fast Brotli compression for HTML and static assets", () => {
  assert.match(testMirrorNginxConfig, /brotli on;/)
  assert.match(testMirrorNginxConfig, /brotli_static on;/)
  assert.match(testMirrorNginxConfig, /brotli_comp_level 4;/)
  assert.match(
    testMirrorNginxConfig,
    /brotli_types[\s\S]*text\/javascript[\s\S]*application\/xml\+rss[\s\S]*application\/manifest\+json/,
  )
  assert.doesNotMatch(testMirrorNginxConfig, /brotli_types[\s\S]*text\/html/)
  assert.doesNotMatch(testMirrorNginxConfig, /brotli_comp_level 11;/)
})
