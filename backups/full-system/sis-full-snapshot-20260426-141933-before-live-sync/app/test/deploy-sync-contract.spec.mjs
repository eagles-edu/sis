...
const liveRuntimeSyncScriptPath = path.resolve(process.cwd(), "tools/sync-and-restart-runtimes.sh")
...
const liveRuntimeSyncScript = fs.readFileSync(liveRuntimeSyncScriptPath, "utf8")
...
test("live restart wrapper keeps DB unchanged and avoids test-only boot-prep semantics", () => {
  assert.match(liveRuntimeSyncScript, /note=DB unchanged \(no migrate, no restore\)/)
  assert.match(liveRuntimeSyncScript, /sync_live_prisma_files\(\)/)
  assert.match(liveRuntimeSyncScript, /refresh_live_prisma_client\(\)/)
  assert.match(liveRuntimeSyncScript, /rsync -a --delete \"\$\{REPO_ROOT\}\/prisma\/\" \"\$\{LIVE_ROOT\}\/prisma\/\"/)
  assert.match(liveRuntimeSyncScript, /rsync -a --delete \"\$\{REPO_ROOT\}\/prisma\.config\.ts\" \"\$\{LIVE_ROOT\}\/\"/)
  assert.match(liveRuntimeSyncScript, /npm run db:generate/)
  assert.doesNotMatch(liveRuntimeSyncScript, /boot-prep/)
  assert.doesNotMatch(liveRuntimeSyncScript, /db:migrate:deploy/)
  assert.doesNotMatch(liveRuntimeSyncScript, /db:restore/)
})
