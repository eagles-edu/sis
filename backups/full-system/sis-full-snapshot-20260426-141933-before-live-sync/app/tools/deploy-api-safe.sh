...
collect_prisma_drift() {
  local source_prisma_dir="${SOURCE_ROOT}/prisma"
  local runtime_prisma_dir="${RUNTIME_ROOT}/prisma"
  local source_prisma_config="${SOURCE_ROOT}/prisma.config.ts"
...
perform_sync() {
  ...
  rsync -a --delete "${RSYNC_EXCLUDES[@]}" "${SOURCE_ROOT}/prisma/" "${RUNTIME_ROOT}/prisma/"
  rsync -a --delete "${SOURCE_ROOT}/prisma.config.ts" "${RUNTIME_ROOT}/"
