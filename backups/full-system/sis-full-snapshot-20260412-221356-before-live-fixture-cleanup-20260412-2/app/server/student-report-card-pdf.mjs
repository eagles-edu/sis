...
import { summarizeAttendanceRows } from "./attendance-summary.mjs"
...
export async function generateStudentReportCardPdf(student, filters = {}) {
  ...
  const attendanceSummary = summarizeAttendanceRows(selected.attendanceRecords)
  ...
  printKeyValue(
    doc,
    "Breakdown",
    `Present (incl. tardy): ${attendanceSummary.present}, Absent: ${attendanceSummary.absent}, Late: ${attendanceSummary.late} (Tardy10: ${attendanceSummary.tardy10}, Tardy30: ${attendanceSummary.tardy30}), Excused: ${attendanceSummary.excused}`
  )
