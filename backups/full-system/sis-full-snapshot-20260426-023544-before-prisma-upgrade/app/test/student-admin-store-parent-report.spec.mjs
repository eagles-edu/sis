...
test("student news save keeps open-date restriction for new rows and allows revise-mode resubmits through the current quarter", () => {
  const source = fs.readFileSync(new URL("../server/student-admin-store.mjs", import.meta.url), "utf8")
  ...
  assert.match(source, /const currentWeekStart = startOfWeekSunday\(nowDate\)/)
  assert.match(source, /const weeklyResubmitCutoff = new Date\(currentWeekStart\.getTime\(\) \+ \(ONE_DAY_MS \* 7\)\)/)
  ...
  assert.match(source, /const quarterWindow = resolveStudentNewsQuarterWindow\(nowDate\)/)
  assert.match(source, /const isCurrentQuarterReportDate =\s*reportDate >= quarterWindow\.startAt && reportDate <= quarterWindow\.endAt/)
  assert.match(source, /const isRevisionRequested = existingStatus === STUDENT_NEWS_REVIEW_STATUS_REVISION_REQUESTED/)
  assert.match(source, /assertWithStatus\(\s*isRevisionRequested\s*\?\s*isCurrentQuarterReportDate\s*:\s*isBeforeWeeklyResubmitCutoff && isCurrentWeekReportDate,\s*403,\s*"News report for this date is locked"\s*\)/s)
...
test("student news review queue keeps DB-native review persistence guards", () => {
  ...
  assert.match(source, /buildStudentNewsFallbackOverlayIndex\(/)
  assert.match(source, /resolveStudentNewsFallbackReviewOverlay\(/)
  assert.match(source, /isStudentNewsReviewSchemaUnavailableError\(/)
  assert.match(source, /prisma\.studentNewsReport\.update\(/)
  assert.match(source, /const fallbackReviewOverlayIndex =\s*reviewSchemaUnavailable && !requiresFallback/s)
  assert.match(source, /if \(!updatedReport\) \{\s*const studentRefId = normalizeText\(existingReport\?\.studentRefId\)/s)
  assert.match(source, /upsertStudentNewsReportInFallbackStore\(studentRefId,\s*reportDateKey,\s*\{/s)
