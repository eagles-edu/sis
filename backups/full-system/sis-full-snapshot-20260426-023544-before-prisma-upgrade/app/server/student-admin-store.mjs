...
function resolveStudentAdminUiSettingsFilePath() {
  return path.resolve(
    process.cwd(),
...
function normalizeStudentNewsSchoolSetupQuarters(value) {
  ...
  entries.forEach((entry) => {
    ...
    if (!quarter || !parseLocalDateOnly(startDate) || !parseLocalDateOnly(endDate)) return
...
function readStudentNewsSchoolSetupFromUiSettingsFile() {
  const filePath = resolveStudentAdminUiSettingsFilePath()
  try {
    const raw = fs.readFileSync(filePath, "utf8")
    if (!normalizeText(raw)) return { schoolYear: "", quarters: [] }
    const parsed = JSON.parse(raw)
    const wrapped = parsed?.uiSettings && typeof parsed.uiSettings === "object" ? parsed.uiSettings : parsed
    const schoolSetup = wrapped?.schoolSetup && typeof wrapped.schoolSetup === "object" ? wrapped.schoolSetup : {}
    return {
      schoolYear: normalizeText(schoolSetup.schoolYear),
      quarters: normalizeStudentNewsSchoolSetupQuarters(schoolSetup.quarters),
    }
  } catch {
...
function resolveStudentNewsQuarterWindowFromDate(nowDate = new Date()) {
  const shifted = shiftToFixedTimeZone(nowDate instanceof Date ? nowDate : new Date(nowDate))
  const year = shifted.getUTCFullYear()
  const month = shifted.getUTCMonth() + 1
  if (month >= 8 && month <= 10) {
    return { quarter: "q1", startDate: `${year}-08-01`, endDate: `${year}-10-31` }
  }
  if (month >= 11 || month <= 1) {
    const startYear = month >= 11 ? year : year - 1
    const endYear = month >= 11 ? year + 1 : year
    return { quarter: "q2", startDate: `${startYear}-11-01`, endDate: `${endYear}-01-31` }
  }
  if (month >= 2 && month <= 4) {
    return { quarter: "q3", startDate: `${year}-02-01`, endDate: `${year}-04-30` }
  }
  return { quarter: "q4", startDate: `${year}-05-01`, endDate: `${year}-07-31` }
}

export function resolveStudentNewsQuarterWindow(now = new Date()) {
  const nowDate = parseDateOrNull(now) || new Date()
  const todayKey = toLocalIsoDate(nowDate)
  const setup = readStudentNewsSchoolSetupFromUiSettingsFile()
  const configuredQuarter = (Array.isArray(setup.quarters) ? setup.quarters : []).find(
    (entry) => todayKey >= entry.startDate && todayKey <= entry.endDate
  )
  const window = configuredQuarter || resolveStudentNewsQuarterWindowFromDate(nowDate)
  const startAt = parseLocalDateOnly(window.startDate) || startOfDay(nowDate)
  const endAt = endOfDay(parseLocalDateOnly(window.endDate) || nowDate)
  return {
    quarter: normalizeQuarter(window.quarter) || "",
...
export async function saveStudentNewsReport(
  ...
  if (existing) {
    ...
    const quarterWindow = resolveStudentNewsQuarterWindow(nowDate)
    const isCurrentQuarterReportDate =
      reportDate >= quarterWindow.startAt && reportDate <= quarterWindow.endAt
    const isRevisionRequested = existingStatus === STUDENT_NEWS_REVIEW_STATUS_REVISION_REQUESTED
    const isApproved = existingStatus === STUDENT_NEWS_REVIEW_STATUS_APPROVED
    assertWithStatus(
      isRevisionRequested
        ? isCurrentQuarterReportDate
