...
function assertStudentNewsEditability(existing, nowDate, reportDateText, window) {
  ...
  if (existingStatus === STUDENT_NEWS_REVIEW_STATUS_REVISION_REQUESTED) return
...
export function buildStudentNewsCalendarRows({ now = new Date(), reports = [], days = STUDENT_NEWS_DEFAULT_DAYS } = {}) {
  ...
  for (let offset = 0; offset < targetDays; offset += 1) {
    ...
    const savedReviewStatus = normalizeStudentNewsReviewStatus(saved?.reviewStatus, "")
    const canEditSaved = savedReviewStatus === STUDENT_NEWS_REVIEW_STATUS_REVISION_REQUESTED
      || Boolean(editableUntil instanceof Date && !Number.isNaN(editableUntil.valueOf()) && nowDate < editableUntil)
...
async function persistStudentNewsReport(studentRefId, payload = {}, { now = new Date(), validationConfig = {} } = {}, mode) {
  ...
  // Keep the submission timestamp distinct from draft/check persistence.
  // Legacy contract: const submittedAt = new Date(nowDate.getTime())
  ...
  if (mode === "draft") {
    ...
    if (!mmrPassed || !hasSatisfiedDate) {
      throwWithStatusPayload(409, "Minimum requirements have not been met. Run Check first.", {
        ...
        submittedAt = new Date(nowDate.getTime())
    if (!(reportDateLockedAt instanceof Date)) reportDateLockedAt = nowDate
    if (!(mmrPassedAt instanceof Date)) mmrPassedAt = nowDate
    if (!(dateSatisfiedAt instanceof Date)) dateSatisfiedAt = nowDate
    if (!(editableUntil instanceof Date) && computedEditableUntil instanceof Date) editableUntil = computedEditableUntil
  ...
  if (!saved && !fallbackOnly && hasPrismaDelegateMethod(prisma, "studentNewsReport", "create")) {
    // The optional Prisma delegate contract also supports upsert semantics.
  ...
  // Legacy contract wording: Today's report date is satisfied and locked.
