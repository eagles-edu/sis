...
test("New Words and News vocabulary use the same full ESL row payload without student MW fill", () => {
  ...
  assert.match(sharedVocabularyEditor, /<input type="hidden" name="vocabularyEsl-etymology-\$\{uid\}" data-vocabulary-esl-field="etymology">/)
  assert.match(sharedVocabularyEditor, /Origin path<input[^>]*data-vocabulary-origin-field="originPath"/)
  assert.match(sharedVocabularyEditor, /<input type="hidden" name="vocabularyEsl-etymology-\$\{uid\}" data-vocabulary-esl-field="etymology">/)
  assert.match(sharedVocabularyEditor, /const paragraph = String\(preview\.paragraph \|\| ""\)\.trim\(\);[\s\S]*?ET preview loaded\.[\s\S]*?return;/)
  assert.doesNotMatch(sharedVocabularyEditor, /preview\.originPath\) pathInput\.value|preview\.reference \? \[\.\.\.references, preview\.reference\]/)
  assert.doesNotMatch(sharedVocabularyEditor, /const textarea = row\.querySelector\('\[data-vocabulary-field="definition"\]'\)[\s\S]*?preview\.paragraph/)
  assert.match(libraryReviewWorkbench, /if \(selectedCandidates\[datum\] \|\| datum === "originPath"\) return/)
...
test("animated SVG custom elements preserve animation markup across rerenders", () => {
...
test("admin Library is a protected physical page under Administration without chat", () => {
  ...
  assert.match(admin, /function renderAccordionEntry\(accordion\)[\s\S]*?accordion\.addEventListener\("toggle", \(\) => renderAccordionEntry\(accordion\)\)/)
  assert.match(admin, /accordions\.every\(\(accordion\) => accordion\.dataset\.libraryEntryId\)/)
  ...
  assert.match(libraryReviewWorkbench, /button\.closest\("\[data-vocabulary-flat-entry\], \[data-review-pane\], \[data-vocabulary-editor\], \[data-approved-editor\]"\)/)
  ...
  assert.match(sharedVocabularyEditor, /portal-button-blue-action portal-button-compact" data-vocabulary-transitivity-check/)
  assert.match(sharedVocabularyEditor, /portal-button-blue-action portal-button-compact" data-vocabulary-transitivity-autofill/)
  ...
  assert.match(admin, /payload\.dictionaryMetadata = \{\s*\.\.\.[\s\S]*audioInputs/u)
...
test("admin Library editor uses the shared New Words row renderer", () => {
  ...
  assert.match(sharedVocabularyEditor, /const dictionaryBuilderSourceSession = includeDictionaryBuilder[\s\S]*?data-dictionary-builder-open-sources/)
  assert.match(sharedVocabularyEditor, /const sourceActionButtons = `\$\{dictionaryBuilder\}[\s\S]*?\$\{includeDictionaryBuilder \? transitivityTools : ""\}`/)
  assert.match(sharedVocabularyEditor, /const sourceActions = [\s\S]*?\? `\$\{sourceMessages\}<div class="vocabulary-source-control-layout"><div class="vocabulary-source-actions">\$\{sourceActionButtons\}<\/div>\$\{dictionaryBuilderSourceSession\}/)
  assert.match(sharedVocabularyEditor, /const standaloneTransitivityTools = includeDictionaryBuilder \? "" : transitivityTools/)
  assert.match(sharedPortalTheme, /\.vocabulary-source-control-layout\s*\{[\s\S]*?grid-template-columns:\s*minmax\(280px, 326px\) minmax\(120px, 138px\) minmax\(0, 1fr\)/)
  assert.match(sharedPortalTheme, /\.vocabulary-source-control-layout \.dictionary-builder-source-startup-row\s*\{[\s\S]*?display:\s*contents/)
  assert.match(sharedPortalTheme, /\.vocabulary-source-control-layout \.vocabulary-source-actions \.vocabulary-transitivity-check\s*\{[\s\S]*?display:\s*contents/)
  assert.match(sharedPortalTheme, /\.dictionary-builder-source-leds\s*\{[\s\S]*?border:\s*1px solid var\(--portal-border\)[\s\S]*?min-height:\s*36px/)
  assert.match(sharedPortalTheme, /\.dictionary-builder-source-leds\s*\{[^}]*flex-wrap:\s*wrap[^}]*min-height:\s*36px[^}]*overflow:\s*hidden/)
  assert.doesNotMatch(sharedPortalTheme, /\.dictionary-builder-source-leds\s*\{[^}]*overflow-x:\s*auto/)
  assert.match(sharedPortalTheme, /\.dictionary-builder-source-led\s*\{[\s\S]*?flex:\s*0 0 32px[\s\S]*?height:\s*32px[\s\S]*?width:\s*32px/)
  assert.match(sharedPortalTheme, /\.dictionary-builder-source-bank\s*\{[\s\S]*?padding:\s*6px 8px/)
  assert.match(sharedPortalTheme, /\.dictionary-builder-source-led-legend\s*\{[\s\S]*?font-size:\s*\.82rem/)
...
test("admin verb editor keeps headword audio on row two and six exact verb-form columns", () => {
  ...
  assert.doesNotMatch(sharedPortalTheme, /\.vocabulary-verb-form-editor \.vocabulary-audio-editor\s*\{[^}]*display: contents/)
})
...
test("all standalone admin navigation shells expose the Library child menu", () => {
  for (const [name, html] of standaloneAdminNavs) {
    ...
    assert.match(html, /href="\/admin\/library\/references"/i, name)
    assert.match(html, /href="\/admin\/library\/definitions"/i, name)
  }
})
