...
test("Dictionary Builder does not offer verb forms or verb-form audio for non-verbs", () => {
  const result = normalizeProviderPreview("oxford_ame", {
    ok: true,
    sourceUrl: "https://example.test/language",
    fields: {
      verbForms: { verbV1: "language" },
      verbFormAudio: { verbV1: { us: "https://media.example.test/language.mp3" } },
    },
    entries: [{ headword: "language", partOfSpeech: "noun", inflections: ["languages"], senses: [] }],
  }, { english: "language", partOfSpeech: "noun" })
  assert.equal(result.fields.verbForms, "")
  assert.deepEqual(result.fields.verbFormAudio, {})
  assert.equal(result.datumStatus.verbForms.status, "not_offered")
  assert.equal(result.datumStatus.verbFormAudio.status, "not_offered")
})

test("Dictionary Builder omits non-verb form tabs and ignores stale non-verb form selections on Apply", async () => {
  const source = await fs.readFile(path.join(process.cwd(), "web-asset/admin/library-review-workbench.js"), "utf8")
  assert.match(source, /activeDictionaryBuilderFields = dictionaryBuilderFields\.filter\(\(\[datum\]\) => partOfSpeech === "verb" \|\| !\["verbForms", "verbFormAudio"\]\./u)
  const corpus = await fs.readFile(path.join(process.cwd(), "src/modules/admin/library-corpus.mjs"), "utf8")
  assert.match(corpus, /\["verbForms", "verbFormAudio"\]\.includes\(DICTIONARY_BUILDER_FIELD_TO_DATUM\[field\]\) && lower\(existing\.partOfSpeech\) !== "verb"/u)
})
...
test("every Builder datum has preferred providers that advertise that datum", () => {
  const manifestById = new Map(DICTIONARY_BUILDER_MANIFEST.map((source) => [source.id, source]))
  for (const datum of DICTIONARY_BUILDER_DATUMS) {
    const providers = DICTIONARY_BUILDER_PREFERRED_DATUM_PROVIDERS[datum]
    assert.ok(providers?.length, `${datum} has no preferred providers`)
    assert.ok(providers.every((provider) => {
      const source = manifestById.get(provider) || (provider === "merriam_webster_scrape" ? manifestById.get("merriam_webster") : null)
      return source?.capabilities?.[datum]
    }), `${datum} has a provider without capability integration`)
  }
})
...
test("Dictionary Builder keeps Origin path ordered from MW/AP before Wiktionary", async () => {
  const unsupported = (provider) => async () => ({ provider, status: "unsupported", fields: {}, entries: [], media: [], datumStatus: {} })
  const fetcher = Object.fromEntries(DICTIONARY_BUILDER_MANIFEST.map((source) => [source.id, unsupported(source.id)]))
  fetcher.merriam_webster_api = async () => ({ provider: "merriam_webster_api", status: "available", fields: { originPath: "Middle English → English" }, entries: [], media: [], datumStatus: { originPath: { status: "available" } } })
  const { previewDictionaryBuilder } = await import("../src/modules/admin/dictionary-builder.mjs")
  const snapshot = await previewDictionaryBuilder(
    { id: "entry-origin-order", english: "chair", partOfSpeech: "noun" },
    { ownerKey: "session-origin-order", fetcher, rankedSources: [], rankedSourcesByDatum: Object.fromEntries(DICTIONARY_BUILDER_DATUMS.map((datum) => [datum, []])) },
  )
  assert.deepEqual(snapshot.datumSourceOrder.originPath, ["merriam_webster_api", "merriam_webster_scrape"])
  assert.equal(snapshot.sources.find((source) => source.provider === "merriam_webster_api").datumStatus.originPath.status, "available")
})
...
test("MW Thesaurus separates antonyms from a combined relation container", async () => {
  const result = await previewHtmlAdapter("merriam_webster_thesaurus", { english: "clean", partOfSpeech: "adjective" }, async () => ({
    ok: true,
    url: "https://www.merriam-webster.com/thesaurus/clean",
    text: async () => '<main><section class="thesaurus-pos"><h2>adjective</h2><div class="synonyms-and-antonyms"><div class="synonym-list"><ul><li>neat</li><li>tidy</li></ul></div><div class="antonym-list"><ul><li>dirty</li><li>messy</li></ul></div></div></section></main>',
  }))
  assert.equal(result.fields.synonymsAntonyms, "Synonyms:\nneat\ntidy\n\nAntonyms:\ndirty\nmessy")
})
