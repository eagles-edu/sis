...
test("dictionary scraper integrations preserve protected media and audit contracts", () => {
  ...
  assert.match(workbench, /Save the canonical Library entry before applying data\./)
  ...
  for (const token of ["Open sources", "INITIAL_SOURCE_GROUP_SIZE = 6", "INITIAL_SOURCE_LED_ORDER", "dictionary-builder-source-leds", "dictionary-builder-source-led-legend", "data-dictionary-builder-open-all", "bindEditorSourceStartup", "sourceLedOverrides", "sourceLedContainers", "renderAllSourceLeds", "sourceBrowserTabs", "closeSourceBrowserTabs", "cookie/robot prompt", "this datum is paused", "Retry provider", "provider tabs", "native cookie jar"]) assert.ok(workbench.includes(token), `missing workbench contract: ${token}`)
  ...
  assert.match(workbench, /INITIAL_SOURCE_PROVIDER_ORDER = \["ldoce", "oxford_ame", "merriam_webster_thesaurus", "britannica", "merriam_webster_scrape", "wordhelp"\]/)
