(() => {
  ...
  const dictionaryBuilderPreferredProviders = Object.freeze({
    ...
    firstKnownUse: ["merriam_webster_api", "merriam_webster_scrape"], originPath: ["merriam_webster_api", "merriam_webster_scrape"], etymology: ["etymonline", "merriam_webster_api", "wiktionary"], worksCited: ["etymonline"],
  })
  ...
  const dictionaryBuilderDialog = () => {
    ...
    if (!dialog) {
      ...
      dialog.innerHTML = `<form method="dialog" class="library-dictionary-builder-form"><header class="library-dictionary-builder-header"><div data-dictionary-builder-source-matrix-slot></div><div class="library-dictionary-builder-header-main"><h2 id="libraryDictionaryBuilderTitle" data-dictionary-builder-tab-heading></h2><p class="small" data-dictionary-builder-section-summary aria-live="polite"></p><p class="small" data-dictionary-builder-message aria-live="polite"></p></div><button type="submit" class="portal-button portal-button-neutral-action" aria-label="Close Dictionary Builder" data-button-tooltip-title="Close the Dictionary Builder without applying staged data." title="Close the Dictionary Builder without applying staged data.">Close</button></header><details open class="dictionary-builder-acquisition-log"><summary>Acquisition log</summary><pre data-dictionary-builder-log aria-live="polite"></pre></details><div data-dictionary-builder-root></div></form>`
      dialog.addEventListener("close", () => {
        closeSourceBrowserTabs()
        sourceLedOverrides.clear()
        renderAllSourceLeds()
      })
      document.body.append(dialog)
    }
  ...
  const INITIAL_SOURCE_PROVIDER_ORDER = ["ldoce", "oxford_ame", "merriam_webster_thesaurus", "britannica", "merriam_webster_scrape", "wordhelp"]
  const INITIAL_SOURCE_LED_ORDER = ["ldoce", "oxford_ame", "merriam_webster_thesaurus", "britannica", "merriam_webster_scrape", "wordhelp"]
  const INITIAL_SOURCE_LED_LABELS = Object.freeze({ ldoce: "LD", oxford_ame: "OA", merriam_webster_thesaurus: "TH", britannica: "BR", merriam_webster_scrape: "MW", wordhelp: "WH" })
  const sourceLedOverrides = new Map()
  const sourceLedContainers = new Set()
  ...
  const renderAllSourceLeds = () => {
    sourceLedContainers.forEach((container) => {
      if (!container.isConnected) sourceLedContainers.delete(container)
      else renderSourceLeds(latestSourceSnapshot, container)
    })
  }
  const sourceStatusForLed = (provider, snapshot) => {
    const override = sourceLedOverrides.get(provider)
    ...
    if (["robot_blocked", "cookie_prompt", "robot_prompt", "blocked"].includes(status)) return "popup-blocking"
    return "default"
  }
  const renderSourceLeds = (snapshot, container) => {
    ...
    container.replaceChildren(...INITIAL_SOURCE_LED_ORDER.map((provider) => {
      ...
      const status = sourceStatusForLed(provider, snapshot)
      ...
      led.setAttribute("aria-label", `${INITIAL_SOURCE_LED_LABELS[provider]} status: ${status === "success" ? "full scrape succeeded" : status === "not-found" ? "404 not found" : status === "popup-blocking" ? "popup or source blocked" : status === "opening" ? "opening" : "default"}`)
      return led
    }))
  }
  const setSourceLedStatus = (provider, status, snapshot, container) => {
    sourceLedOverrides.set(provider, status)
    if (snapshot) latestSourceSnapshot = snapshot
    if (container) sourceLedContainers.add(container)
    renderAllSourceLeds()
  }
  const bindEditorSourceStartup = (pane) => {
    ...
    pane.addEventListener("focusin", (event) => {
      if (event.target.matches("input, textarea, select")) pane.__dictionaryBuilderLastEditorFocus = event.target
    })
    sourceLedContainers.add(container)
    ...
    startup.addEventListener("click", () => {
      ...
      pane.__dictionaryBuilderLastEditorFocus?.focus?.({ preventScroll: true })
    })
  }
  const renderDictionaryBuilder = (snapshot, pane, sourceId, previousSelections = {}, activeTab = "vietnamese", initialReservation = null) => {
    ...
    const activeDictionaryBuilderFields = dictionaryBuilderFields.filter(([datum]) => partOfSpeech === "verb" || !["verbForms", "verbFormAudio"].includes(datum))
    const tabs = activeDictionaryBuilderFields.map(([datum, label], index) => `<button type="button" class="dictionary-builder-tab" id="dictionary-builder-tab-${datum}" role="tab" aria-controls="dictionary-builder-panel-${datum}" aria-selected="${index === 0}" tabindex="${index === 0 ? 0 : -1}" data-dictionary-builder-tab="${datum}"><span class="dictionary-builder-tab-step">${index + 1}</span><span>${dictionaryBuilderEsc(label)}</span></button>`).join("")
    ...
    root.innerHTML = `<div class="dictionary-builder-source-bank"><div class="dictionary-builder-source-startup-row"><button type="button" class="portal-button portal-button-blue-action portal-button-compact dictionary-builder-source-startup" data-dictionary-builder-open-all data-button-tooltip-title="Open the six initial browser dictionary sources in named tabs; AP uses the protected API and does not need a browser tab." title="Open the six initial browser dictionary sources in named tabs; AP uses the protected API and does not need a browser tab.">Open sources</button><div class="dictionary-builder-source-status-block"><div class="dictionary-builder-source-leds" data-dictionary-builder-source-leds role="group" aria-label="Initial source status LEDs"></div><div class="dictionary-builder-source-led-legend" aria-label="Source LED legend"><span><i data-led-legend="default" aria-hidden="true"></i>Default</span><span><i data-led-legend="not-found" aria-hidden="true"></i>404</span><span><i data-led-legend="opening" aria-hidden="true"></i>Opening</span><span><i data-led-legend="popup-blocking" aria-hidden="true"></i>Popup blocked</span><span><i data-led-legend="success" aria-hidden="true"></i>Full scrape</span></div></div></div></div><div class="dictionary-builder-tabs" role="tablist" aria-label="Dictionary Builder sections">${tabs}</div><div data-dictionary-builder-panels></div><div class="library-dictionary-preview-actions"><select data-dictionary-builder-mode aria-label="Dictionary Builder apply mode"${canApply ? "" : " disabled"}><option value="fill_missing">Fill missing</option><option value="replace_selected">Replace selected</option><option value="replace_all">Replace all</option></select><button type="button" class="portal-button portal-button-affirm" data-dictionary-builder-apply data-button-tooltip-title="Apply the selected verified datum values and download selected protected audio." title="Apply the selected verified datum values and download selected protected audio."${canApply ? "" : " disabled"}>Apply</button></div><div data-dictionary-builder-popup-fallbacks hidden></div>`
    const sourceLedContainer = root.querySelector("[data-dictionary-builder-source-leds]")
    sourceLedContainers.add(sourceLedContainer)
    latestSourceSnapshot = snapshot
    renderAllSourceLeds()
    const panels = root.querySelector("[data-dictionary-builder-panels]")
    const selectedCandidates = Object.fromEntries(Object.entries(structuredClone(previousSelections)).filter(([datum]) => activeDictionaryBuilderFields.some(([candidateDatum]) => candidateDatum === datum)))
    activeDictionaryBuilderFields.forEach(([datum]) => {
      if (selectedCandidates[datum] || datum === "originPath") return
    ...
    const openInitialSourceGroup = () => {
      const focusedControl = dialog.querySelector("input:focus, textarea:focus, select:focus")
      const sourceByProvider = new Map((snapshot.sources || []).map((source) => [source.provider, source]))
      INITIAL_SOURCE_PROVIDER_ORDER.forEach((provider) => {
        if (sourceStatusForLed(provider, snapshot) === "default") setSourceLedStatus(provider, "opening", snapshot, sourceLedContainer)
      })
      ...
      pendingNavigations.forEach(({ tab, sourceUrl }, index) => {
        window.setTimeout(() => {
          if (!tab.closed) tab.location.href = sourceUrl
        }, index * 1000)
      })
      ...
      dialog.querySelector("[data-dictionary-builder-message]").textContent = opened === initialSourceEntries.length
        ? `Initial LD / OA / TH / BR / MW / WH group opened as named tabs in this browser session. AP uses the protected Merriam-Webster API and does not need a browser tab. Resolve any cookie/robot prompts; affected datums remain paused until Retry provider. Cookies remain in the browser's native cookie jar.`
      ...
      window.setTimeout(() => {
        ...
        if (!focusedControl) pane.__dictionaryBuilderLastEditorFocus?.focus?.({ preventScroll: true })
      }, 500)
    }
    ...
    if (initialReservation) {
      ...
      if (reservationMessage) reservationMessage.textContent = initialReservation.blocked.length
        ...
        : `Opened ${initialReservation.reserved.size} of ${INITIAL_SOURCE_GROUP_SIZE} initial provider tabs in order LD / OA / TH / BR / MW / WH. AP uses the protected Merriam-Webster API and does not need a browser tab. Resolve cookie/robot prompts in those tabs; cookies remain active for this Builder session.`
    ...
    const show = (datum, focus = false) => {
      ...
      retryPreferred.textContent = "Retry"
      retryPreferred.title = `Wait 5 seconds, then check the saved snapshot first and retry the preferred sources (${preferredProviders.join(", ")}) for ${datum}.`
      retryPreferred.setAttribute("data-button-tooltip-title", retryPreferred.title)
      ...
      retryPreferred.addEventListener("click", async () => {
        ...
        try {
          ...
          sourceLedOverrides.clear()
          latestSourceSnapshot = refreshed
          renderAllSourceLeds()
          renderDictionaryBuilder(refreshed, pane, sourceId, {}, datum)
        } catch (error) {
        ...
        if (isDictionaryBuilderPromptStatus(candidateStatus)) {
          ...
          const retryProvider = async ({ automatic = false } = {}) => {
            ...
            try {
              ...
              const datumReady = refreshedStatus === "available" && refreshedValue !== undefined && refreshedValue !== null && String(refreshedValue).trim() !== ""
              if (automatic && !datumReady) return false
              stopPromptProbe()
              sourceLedOverrides.delete(candidate.provider)
    ...
    show(activeDictionaryBuilderFields.some(([datum]) => datum === activeTab) ? activeTab : activeDictionaryBuilderFields[0][0])
    if (!canApply) {
      const message = dialog.querySelector("[data-dictionary-builder-message]")
    ...
    document.querySelectorAll("[data-vocabulary-dictionary-builder]").forEach((button) => {
      ...
      button.addEventListener("click", async () => {
        ...
        try {
          ...
          const editorSourceLeds = pane.querySelector("[data-dictionary-builder-source-leds]")
          if (editorSourceLeds) sourceLedContainers.add(editorSourceLeds)
          latestSourceSnapshot = snapshot
          renderAllSourceLeds()
