...
test("Builder renders every datum with a manual path and POS dropdowns", () => {
  ...
  assert.match(workbench, /window\.open\(candidate\.sourceUrl, "_blank"\)[\s\S]*Leave it open[\s\S]*setInterval[\s\S]*retryProvider\(\{ automatic: true \}\)/, "Robot-blocked sources stay open while Builder probes automatically")
