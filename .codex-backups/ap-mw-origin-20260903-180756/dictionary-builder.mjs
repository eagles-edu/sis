...
export function normalizeProviderPreview(provider, preview, entry) {
  ...
  const verbFormAudio = Object.fromEntries(audioEntries.flatMap((item) => item?.verbFormAudio && typeof item.verbFormAudio === "object" ? Object.entries(item.verbFormAudio) : []).filter(([, values]) => values?.us).map(([slot, values]) => [slot, values]))
  const normalizedVerbFormAudio = Object.keys(verbFormAudio).length ? verbFormAudio : fields.verbFormAudio && typeof fields.verbFormAudio === "object" ? fields.verbFormAudio : {}
