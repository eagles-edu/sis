...
import { previewDictionaryBuilder, readDictionaryBuilderSnapshot } from "../src/modules/admin/dictionary-builder.mjs"
...
function createDefaultRolePermissions() {
  return {
    ...
    teacher: {
      ...
      startPage: "overview",
      allowedPages: ["overview", "profile", "attendance", "assignments", "parent-tracking", "performance-engagement", "grades", "reports", "family"],
    },
    student: {
      role: "student",
      canRead: true,
...
function setInitialAdminAuthState(html, authenticated) {
  ...
  let nextHtml = toggleById(html, "authBootPanel", true)
  nextHtml = toggleById(nextHtml, "authPanel", authenticated)
  return toggleById(nextHtml, "app", !authenticated)
}
...
function injectAdminRuntimeConfig(html, pageSlug, origin, initialAuthState = { authenticated: false }) {
  ...
  const htmlWithInitialAuth = setInitialAdminAuthState(htmlWithBodyClass, normalizedAuthState.authenticated)
  const htmlWithInitialSection = normalizedAuthState.authenticated ?
    setInitialAdminLayoutState(setInitialActivePageSection(htmlWithInitialAuth, pageSlug), pageSlug)
    : htmlWithInitialAuth
...
async function handleApiRequest(request, response, pathname, url) {
  ...
  if (method === "POST" && pathname === ADMIN_NOTIFY_BATCH_STATUS_PATH) {
    ...
    if (action === "delete") {
      const result = await deleteQueuedAnnouncement(queueId)
      sendJson(response, 200, { ok: true, action: "delete", ...result })
      return true
    }

    if (action === "unqueue") {
      const result = await unqueueQueuedAnnouncement(queueId, {
        returnedByUsername: normalizeText(session?.username),
      })
      sendJson(response, 200, { ok: true, action: "unqueue", ...result })
      return true
    }
