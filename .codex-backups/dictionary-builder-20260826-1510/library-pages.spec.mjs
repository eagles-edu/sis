...
test("admin Library is a protected physical page under Administration without chat", () => {
  ...
  assert.match(admin, /editorRowHtml\(`review-\$\{slot\}`, \{ includeLdoce: true, includeOxford: true, includeBritannica: true, includeMerriamWebster: true, includeTransitivityTools: true, includeOriginAnalysis: true, originLookupPath:/)
  ...
  assert.match(admin, /let entries = new Map\(\);\s+const flatHtml = \(entry\) => window\.SIS_VOCABULARY_ESL\.flatEntryHtml[\s\S]*?function enhanceAccordions\(\) \{[\s\S]*?async function loadApproved\(\)/)
...
test("admin Library editor uses the shared New Words row renderer", () => {
  ...
  assert.match(sharedVocabularyEditor, /vocabulary-source-actions">\$\{mwFill\}\$\{merriamWebsterFill\}\$\{britannicaFill\}/)
  assert.match(admin, /lookupButtons: \["OE", "BR", "LD", "MW", "GT", "ET", "WH", "TH", "GL"\]/)
