...
- The Library-schema controls are `DictionaryBuilderProviderSetting` and `DictionaryBuilderDatumSetting`; neither model stores credentials.
- Matrix, BIC, and XLSX values are calculated server-side at request time from these controls and the independent provider metrics. Historic `merriam_webster` observations remain intact and feed the MW API history; new MW API and MW scrape observations remain independent.
- The workbook has thirteen source sheets plus `BIC`, `Components`, and `Settings`; all values come from the server-side current matrix.
