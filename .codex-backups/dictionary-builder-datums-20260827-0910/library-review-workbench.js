(() => {
  ...
  const dictionaryBuilderFields = [
    ["vietnamese", "Vietnamese"], ["syllabication", "Syllable / Stress"], ["grammarClassification", "POS Classification"],
  ...
  const dictionaryBuilderApplyFields = new Set(["vietnamese", "syllabication", "grammarClassification", "audio", "definition", "verbInfinitive", "verbV1", "verbV2", "verbV3", "verbV4", "verbV5", "etymology", "originPath", "originReferences", "firstKnownUse", "stems"])
  ...
  const dictionaryBuilderSourceMatrix = (word) => {
    ...
    const links = [
      ...
      ["MW", `https://www.merriam-webster.com/dictionary/${encoded}`], ["ET", `https://www.etymonline.com/search?q=${encoded}`], ["WK", `https://en.wiktionary.org/w/index.php?search=${encoded}`], ["CA", `https://dictionary.cambridge.org/dictionary/english/${encoded}`],
  ...
  const renderDictionaryBuilder = (snapshot, pane, sourceId) => {
    ...
    const show = (datum, focus = false) => {
      ...
      candidates.forEach((candidate) => {
        ...
        input.addEventListener("input", () => { saveCandidate(); sizeDictionaryBuilderTextarea(input) })
        radio.addEventListener("change", saveCandidate)
        label.append(radio, title, input)
        candidateList.append(label)
        sizeDictionaryBuilderTextarea(input)
      })
      if (!candidates.length) {
        const label = document.createElement("label")
        ...
        const input = document.createElement("textarea")
        input.placeholder = `Enter ${datumLabel}`
        input.dataset.dictionaryBuilderCandidateValue = datum
