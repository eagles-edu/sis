...
const ADMIN_PAGE_PATH = process.env.STUDENT_ADMIN_PAGE_PATH || "/admin/students"
const ADMIN_API_PREFIX = process.env.STUDENT_ADMIN_API_PREFIX || "/api/admin"
...
function allowCors(request, response) {
  ...
  else if (reqOrigin && origins.includes(reqOrigin)) allowOrigin = reqOrigin
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        parsed.forEach((entry) => {
          ...
          accounts.push({ username, role, password, passwordHash })
        })
      }
    } catch (error) {
...
function verifyCredentials(username, password) {
  ...
  for (let i = 0; i < accounts.length; i += 1) {
    ...
    return {
      ...
      role: normalizeLower(account.role) || "teacher",
    }
  }
...
function enforceRoleAccess(session, method) {
  const role = normalizeLower(session?.role) || "teacher"
...
async function handleLogin(request, response) {
  ...
  const principal = verifyCredentials(username, password)
  if (!principal) {
...
async function handleApiRequest(request, response, pathname, url) {
  ...
  const session = await requireAuthenticatedSession(request, response)
  enforceRoleAccess(session, method)

  if (method === "GET" && pathname === `${ADMIN_API_PREFIX}/students/import-template.xlsx`) {
  ...
  if (method === "GET" && pathname === `${ADMIN_API_PREFIX}/students`) {
    assertStoreEnabled()
  ...
  if (method === "POST" && pathname === `${ADMIN_API_PREFIX}/students`) {
    assertStoreEnabled()
    const payload = await parseBody(request)
  ...
  if (method === "POST" && pathname === `${ADMIN_API_PREFIX}/students/import`) {
    assertStoreEnabled()
    const payload = await parseBody(request)
  ...
  const reportCardPathMatch = pathname.match(/^\/api\/admin\/students\/([^/]+)\/report-card\.pdf$/)
  ...
  const studentPathMatch = pathname.match(/^\/api\/admin\/students\/([^/]+)$/)
  if (studentPathMatch) {
    assertStoreEnabled()
  ...
  const attendancePathMatch = pathname.match(/^\/api\/admin\/students\/([^/]+)\/attendance$/)
  ...
  const attendanceDeleteMatch = pathname.match(/^\/api\/admin\/students\/([^/]+)\/attendance\/([^/]+)$/)
  ...
  const gradePathMatch = pathname.match(/^\/api\/admin\/students\/([^/]+)\/grades$/)
  ...
  const gradeDeleteMatch = pathname.match(/^\/api\/admin\/students\/([^/]+)\/grades\/([^/]+)$/)
  ...
  const reportPathMatch = pathname.match(/^\/api\/admin\/students\/([^/]+)\/reports$/)
  ...
  const reportGenerateMatch = pathname.match(/^\/api\/admin\/students\/([^/]+)\/reports\/generate$/)
  ...
  const reportDeleteMatch = pathname.match(/^\/api\/admin\/students\/([^/]+)\/reports\/([^/]+)$/)
...
export async function handleStudentAdminRequest(request, response) {
  ...
  if (method === "GET" && pathname === ADMIN_PAGE_PATH) {
