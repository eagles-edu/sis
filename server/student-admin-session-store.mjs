...
function makeSessionPayload(id, principal, ttlSeconds) {
  ...
  return {
    ...
    accountId: normalizeText(principal?.accountId),
    eaglesId: normalizeText(principal?.eaglesId),
    parentsId: normalizeText(principal?.parentsId),
    posthogDistinctId: normalizeText(principal?.posthogDistinctId),
    studentRefId: normalizeText(principal?.studentRefId),
    createdAt: now,
    updatedAt: now,
    expiresAt,
  }
