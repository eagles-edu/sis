...
4. `tools/sync-and-restart-test-runtime.sh` is a file-whitelist mirror; it does not rely on git commit matching. `full` additionally creates a restorable test-database backup in the same timestamped backup bundle.
...
11. Full sync purges only the test runtime/public mirror files. It does not purge, restore, or truncate the test database; it backs up the database first, then applies Prisma generation/migrations against the existing test database.
12. `.env.test`, `SIS_CONFIG.json`, and `runtime-data/admin-ui-settings.json` are preserved across the runtime purge and restored with hash verification. The full backup manifest records the runtime, public, vhost, and database backup locations.
