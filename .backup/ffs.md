...
## Pair C: Test Host Strict Runtime Deploy

Use this pair only for the clean test runtime at `/home/test.eagles.edu.vn/sis`.
This is the authoritative strict manifest for the test host. Do not widen it back to tree mirrors.
The order matters:

1. Dev/runtime code first.
2. Live HTML pages second.
3. Exact asset files those pages reference third.

- Source (left): `/home/eagles/dockerz/sis`
- Target (right): `/home/test.eagles.edu.vn/sis`
- Sync variant: `Mirror`
...
\web-asset\shared\portal-theme-state.js
\web-asset\shared\portal-navigation.js
\web-asset\shared\portal-theme.min.css
\web-asset\shared\secure-network.svg
...
\web-asset\images\favicon.ico
\web-asset\admin\favicon.ico
\web-asset\icons\web-component\svg-icon.js
\web-asset\icons\web-component\svgs\theme-moon.svg
\web-asset\icons\web-component\svgs\theme-sun.svg
\web-asset\icons\web-component\svgs\joggling-lava.svg
\web-asset\icons\web-component\svgs\joggling-triangles.svg
\web-asset\icons\web-component\svgs\spiral.svg
\web-asset\vendor\fullcalendar\index.global.min.js
\web-asset\vendor\tabulatorz\tabulator.min.css
\web-asset\vendor\tabulatorz\tabulator.min.js
```
...
1. `src`, `server`, `schemas`, and `prisma` are the runtime trees that must stay in sync before Prisma or restart work.
2. `prisma.config.ts`, `package.json`, and `package-lock.json` are copied exactly because the wrapper depends on them for `npm ci`, `prisma generate`, and `prisma migrate deploy`.
3. `portal-theme.min.css` is the runtime CSS file; do not sync `portal-theme.css` unless you are rebuilding assets on the host.
4. `student-admin.min.css` and `student-admin.min.js` are the compiled admin assets used by the runtime admin pages.
5. `theme-moon`, `theme-sun`, `joggling-lava`, `joggling-triangles`, and `spiral` are the only portal icon SVGs currently referenced by the hub, parent, and student portals.
6. `secure-network.svg`, `maintenance.svg`, `logo.svg`, and `images/favicon.ico` are the shared image assets currently referenced by the portals.
7. `images/favicon.ico` is the single red favicon source for hub, parent, and student; the runtime exposes it at `/favicon.ico`.
8. `admin/favicon.ico` is the blue favicon source for admin pages.
9. `web-asset/vendor/fullcalendar/index.global.min.js` and `web-asset/vendor/tabulatorz/tabulator.min.{css,js}` are the vendored runtime assets used by the calendar and grid surfaces.
10. The live HTML set here is `student-admin.html`, `portal-hub.html`, `report-card.html`, `grades-tabulator.html`, `student-points.html`, `parent-portal.html`, and `student-portal.html`.
11. The test wrapper owns `.env.test` seeding, `public_html` mirroring, Prisma refresh, systemd restart, and route verification. It preserves the same immutable runtime files as live so the test sync can expose wipe and mirror regressions before live. Keep those responsibilities out of this FFS pair.
12. The wrapper writes the public mirror from the same strict whitelist, plus `public_html/index.html` derived from `web-asset/admin/portal-hub.html`.
13. The test-host whitelist is the same source whitelist as dev for the mirrored runtime payload; the only difference is target roots plus the public path remap into `sis-admin`, `sis-parent`, and `sis-student`.
14. The live admin clean-sync wrapper uses the same source whitelist as test, preserves `runtime-data/admin-ui-settings.json` and `SIS_CONFIG.json` across the wipe, derives `public_html/index.html` from `web-asset/admin/portal-hub.html` with the live admin origin, clears the managed live roots after backup, verifies the cleared roots are empty, restores the whitelist into empty targets, and then verifies the synced hashes and routes. Test keeps the same immutable runtime files out of the mirrored payload for the same reason: it is the canary that catches unsafe-sync behavior before live. Test and live are identical in file-mirror contract; only the vhost/origin and database wiring differ.
...
REDIS_SESSION_URL
REDIS_CACHE_URL
See [test:redis env note](./test-redis-note.md) for the authenticated Redis URL expectation.
SMTP_FROM
SMTP_HOST
SMTP_PASS
SMTP_PORT
