...
stash_local_sis_config() {
  ...
  SIS_CONFIG_SOURCE_BACKUP_DIR="$(mktemp -d)"
  SIS_CONFIG_SOURCE_BACKUP="${SIS_CONFIG_SOURCE_BACKUP_DIR}/${SIS_CONFIG_FILE_NAME}"
  cp -a "$SIS_CONFIG_SOURCE_PATH" "$SIS_CONFIG_SOURCE_BACKUP"
}

restore_local_sis_config() {
  if [[ -n "$SIS_CONFIG_SOURCE_BACKUP" && -f "$SIS_CONFIG_SOURCE_BACKUP" ]]; then
    cp -a "$SIS_CONFIG_SOURCE_BACKUP" "$SIS_CONFIG_SOURCE_PATH"
    SIS_CONFIG_SOURCE_BACKUP=""
  fi
  if [[ -n "$SIS_CONFIG_SOURCE_BACKUP_DIR" && -d "$SIS_CONFIG_SOURCE_BACKUP_DIR" ]]; then
    rm -rf -- "$SIS_CONFIG_SOURCE_BACKUP_DIR"
    SIS_CONFIG_SOURCE_BACKUP_DIR=""
  fi
}
