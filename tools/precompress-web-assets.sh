...
ADMIN_RUNTIME_ROOT=${ADMIN_RUNTIME_ROOT:-/home/admin.eagles.edu.vn/sis}
TEST_RUNTIME_ROOT=${TEST_RUNTIME_ROOT:-/home/test.eagles.edu.vn/sis}
...
case "${1:-}" in
    admin)
        MIRROR_LABEL=admin
        MIRROR_PUBLIC_ROOT=$ADMIN_ROOT
        MIRROR_RUNTIME_ROOT=$ADMIN_RUNTIME_ROOT
        MIRROR_URL=$ADMIN_URL
        ;;
    test)
        MIRROR_LABEL=test
        MIRROR_PUBLIC_ROOT=$TEST_ROOT
        MIRROR_RUNTIME_ROOT=$TEST_RUNTIME_ROOT
        MIRROR_URL=$TEST_URL
        ;;
    -h|--help)
        usage
...
if [ ! -d "$MIRROR_PUBLIC_ROOT" ]; then
    echo "ERROR: public root does not exist: $MIRROR_PUBLIC_ROOT" >&2
    exit 1
fi

if [ ! -d "$MIRROR_RUNTIME_ROOT/web-asset" ]; then
    echo "ERROR: runtime web-asset root does not exist: $MIRROR_RUNTIME_ROOT/web-asset" >&2
    exit 1
fi
...
compress_root() {
    ...
    if [ "$files" -eq 0 ]; then
        rm -f "$stats_file"
        trap - 0 1 2 3 15
        echo "ERROR: no eligible web assets found in $root" >&2
        echo "       Run the mirror sync first or set the correct *_ROOT variable." >&2
        return 1
    fi
...
verify_encoding() {
    ...
    case "$status" in
        2[0-9][0-9]|3[0-9][0-9]) ;;
        *)
            echo "ERROR: $url returned HTTP $status; cannot verify $expected negotiation" >&2
            rm -f "$headers"
            trap - 0 1 2 3 15
            return 1
            ;;
...
compress_root "$MIRROR_RUNTIME_ROOT/web-asset" "$MIRROR_LABEL sis/web-asset"
compress_root "$MIRROR_PUBLIC_ROOT" "$MIRROR_LABEL public_html"
